...
function References({setModal}) {
    const [records, setRecords] = useState()

    useEffect(async _ => {
        setRecords(await API.getReferences())
    }, [])
    ...
    function Modal({data}) {
        ...
        async function submitForm(event) {
            ...
            if (isNew) {
                await API.addRecord('References', data)
            } else {
                await API.updateRecord('References', data)
            }
        ...
        return (
            <div className='modal-bg'>
                <div className='modal'>
                    ...
                    <h2>{(isNew ? 'New' : 'Edit') + ' References'}</h2>
